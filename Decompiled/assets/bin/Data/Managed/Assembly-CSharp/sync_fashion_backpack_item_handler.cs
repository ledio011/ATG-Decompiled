...
public class sync_fashion_backpack_item_handler
{
	// Token: 0x06001418 RID: 5144 RVA: 0x000825A8 File Offset: 0x000807A8
	public static SprotoTypeBase sync_fashion_backpack_item_request(SprotoTypeBase req)
	{
		sync_fashion_backpack_item.request request = req as sync_fashion_backpack_item.request;
		if (request != null)
		{
			ItemContainer itemContainer = SingletonDontDestoryUnity<GameManager>.Instance.PlayerData.GetItemContainer(ITEM_CONTAINER_TYPE.FASHION_BACKPACK);
			if (itemContainer != null && request.HasGameitems)
			{
				foreach (KeyValuePair<long, gameitem> keyValuePair in request.gameitems)
				{
					...
					if (itemByIndexId != null)
					{
						...
						itemByIndexId.ContainerType = ITEM_CONTAINER_TYPE.FASHION_BACKPACK;
						if (value.HasParm)
						{
